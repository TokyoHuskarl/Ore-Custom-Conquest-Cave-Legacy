...
#Project Age Of Machinery start...........................................
  ...
  (try_for_range,reg1,"spr_art_mangonel","spr_cannonball"),
    ...
    (try_for_range,":troop","trp_foot_gunner","trp_new_array_a"),
      ...
      (val_sub,":time_left",":number"),
    (try_end),
    (item_set_slot,reg1,229,":time_left"),
    (le,":time_left",0),
    (store_sub,":item",reg1,"spr_art_mangonel"),
    (val_add,":item","itm_mangonel"),
    (item_get_slot,":number",reg1,230),
    (troop_add_items,"trp_artillery_officer",":item",":number"),
    (str_store_item_name,s1,":item"),
    (assign,reg2,":number"),
    (display_message,"@{reg2} new {s1}s ready for use!"),
    (item_set_slot,reg1,230,0),
  (try_end),
  ]),
...
# Tutorial:
  (0.1, 0, ti_once, [(map_free,0)], [(dialog_box,"str_tutorial_map1"),
      ## CC-D begin: check whether initializing succeeds
      (store_item_kind_count, ":count", "itm_occc_sauron_helm", "trp_kingdom_7_lord"),#itm_ccc_helm_lichkinghelm->itm_occc_sauron_helm occc
      (try_begin),
        (lt, ":count", 1),
        (display_message, "@{!}Caution: initializing troops failed.", 0xFF0000),
      (try_end),
      ## CC-D end
    ]),
...
#      (try_begin),
#        (store_random_in_range, ":random_no", 0, 100),
#        (lt, ":random_no", 30), #CC-C begin 10->40
  ...
  (0, 0, 24 * 14,
   [
        (try_for_range, ":pretender", pretenders_begin, pretenders_end),
          ...
          (try_for_range, ":unused", 0, 30), #CC-C 30->50  ## CC-D: not use
            ...
            (le, ":relation", 0), #fail if nothing qualifies #CC-C 0->20  ## CC-D: not use
...
#Move unemployed NPCs around taverns
   ...
   [
    ...
    ## CC-D begin
    (call_script, "script_ccd_update_random_npc_payment"),
    ## CC-D end
    ],
   []
   ),
...
#Check for political issues
            ...
            (try_begin),
                ...
                (try_begin),
                    (eq, "$disable_npc_complaints", 0), #CC-C clash 1 to 0
                    (lt, ":npc_morale", 20),
     ...
     (try_for_range, ":companion", companions_begin, companions_end),
       ...
       (try_for_range, ":i_slot", 0, ":inv_cap"),
		 ...
		 (unlock_achievement, ACHIEVEMENT_LADY_OF_THE_LAKE),
		  (call_script, "script_ccd_record_achievement", ACHIEVEMENT_LADY_OF_THE_LAKE),  ## CC-D add: for local achievement
		 (assign, ":inv_cap", 0),
	   (try_end),
	 (try_end),
    ],
...
## NMC begin
## Deactivated, as I use Rubik's autoloot.
...
#CC-C begin
  (0.1, 0, 0, [
    ...
    (this_or_next|check_quest_failed, "qst_ccc_hard_core"),
    ## CC-D end
    (ge,"$g_ccc_option_hard_core_mode",2),], [(jump_to_menu,"mnu_hard_core_dead_end"),]),
#CC-C end
##diplomacy end
