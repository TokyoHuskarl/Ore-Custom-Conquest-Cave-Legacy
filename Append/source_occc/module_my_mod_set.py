...
#      item_data.append((item_set_slot, i_item, slot_item_speed, get_speed_rating(items[i_item][6])))
    ...
    if type == itp_type_polearm and get_weapon_length(items[i_item][6]) >= 150 and (get_thrust_damage(items[i_item][6]) % 256) > (get_swing_damage(items[i_item][6]) % 256):
        item_data.append((item_set_slot, i_item, slot_item_pike, 1))
	
#	# ## couchable - CABA addition
#    if items[i_item][3] & itp_couchable == itp_couchable:
#      item_data.append((item_set_slot, i_item, slot_item_couchable, 1))
#
#    ## needs two hands - CABA addition
#    item_data.append((item_set_slot, i_item, slot_item_needs_two_hands, items[i_item][3] & itp_two_handed))
#
#	## CC-D begin
#    if type == itp_type_crossbow or type == itp_type_pistol or type == itp_type_musket:
#      item_data.append((item_set_slot, i_item, slot_item_load_bullet, get_max_ammo(items[i_item][6])))
#    
#    if items[i_item][3] & itp_unique == itp_unique:
#      item_data.append((item_set_slot, i_item, slot_item_unique, -1))
#    
#    if ((type >= itp_type_arrows and type <= itp_type_thrown and type != itp_type_shield) or (type >= itp_type_pistol and type <= itp_type_bullets)) and (items[i_item][3] & itp_can_penetrate_shield == itp_can_penetrate_shield):
#      item_data.append((item_set_slot, i_item, slot_item_penetrate_shield, 1))
#    ## CC-D end
#
#  ## item_modifier
