    ...
    (ti_on_presentation_load,
     [(set_fixed_point_multiplier, 1000),
	  #(assign, "$g_presentation_credits_obj_1", 0),
	    ...
	    (assign, ":battle_size", reg0),  ## CC-D: not WSE, but 1.161
		(set_show_messages, 0),
		...
		(store_sub, ":battle_size_scaled", ":battle_size", 90), #Scale battle_size to the engine used 30-150 = 0-1 scale occc 30->90
     ...
     [
       ...
       (store_trigger_param_2, ":value"),
		...
		(else_try),
			...
			(overlay_set_color, "$g_presentation_obj_custom_battle_designer_17", 0xFFFFFF),
			(party_set_slot, "p_main_party", slot_party_prebattle_customized_deployment, 0),  ## CC-D fix
		(else_try),
		...
		(else_try), #Number Boxes
		 ...
		 (try_for_range, ":i", 0, ":num_of_stacks"),
			...
			(try_begin),
				...
				(assign, ":selected_troop_id", ":troop_id"),
				(troop_get_slot, ":num_agents", ":troop_id", slot_troop_prebattle_first_round),
				(val_add, ":new_total", ":value"),
			(else_try),
				(troop_get_slot, ":cur_num_agents", ":troop_id", slot_troop_prebattle_first_round),
				(val_add, ":new_total", ":cur_num_agents"),
			(try_end),
			#(assign, ":num_of_stacks", 0), #loop breaker
		 (try_end),
		 
		 (neq, ":selected_troop_id", -1),
		 ...
		 (try_begin),
			...
			#	(store_add, ":new_total", ":cur_count", ":dif"),
			#(try_end),
			(store_sub, ":num_overed", ":new_total", ":round_size"),
			(store_sub, ":modified_value", ":value", ":num_overed"),
			(val_max, ":modified_value", 0),
			
			(store_sub, ":new_total", ":cur_count", ":num_agents"),
			(val_add, ":new_total", ":modified_value"),
			
			(overlay_set_val, ":object", ":modified_value"),
			(troop_set_slot, ":selected_troop_id", slot_troop_prebattle_first_round, ":modified_value"),
		 (else_try),
		    (troop_set_slot, ":selected_troop_id", slot_troop_prebattle_first_round, ":value"),
		 (try_end),
		 ## NMCml end
 ...
 ("pbod_preferences", 0, mesh_load_window, [
    (ti_on_presentation_load,
      [
		...
		(assign, ":num_options", 11),  ## CC-D 15->11
		(val_add, ":num_options", 1), #For extra space for headings  ## CC-D 2->1
		...
		(try_end),		
		(str_store_string, s1, "@{!}Version 0.{s1} (customized)"),  ## CC-D
        (create_text_overlay, reg0, s1, tf_center_justify|tf_with_outline),
        (overlay_set_color, reg0, 0xFFFFFFFF),
		(position_set_x, pos1, 800),
