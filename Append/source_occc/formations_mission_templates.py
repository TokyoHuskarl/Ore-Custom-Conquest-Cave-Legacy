...
###LAG-inducing at high battlesize	
## CC-D begin: PBOD 0.96.3 del
#	(.1, 0, 0, [
...
#implement HOLD OVER THERE when player lets go of key/etc
	(.5, 0, 0, [(neg|main_hero_fallen),(eq, "$g_ccd_pbod_stop_all", 0),], [  ## CC-D add option
		...
		(try_begin),
			...
			(assign, ":radius_enemy_battlegroup", 0), #CABA
			(try_for_range, ":team", 0, 4),
			...
			(val_div, reg0, 2), #CABA - test while 'action_radius' is the full division length
			(try_begin),
				(store_mod, reg1, reg0, 2),
				(neq, reg1, 0),
				(val_add, reg0, 1),
			(try_end), #CABA - end
