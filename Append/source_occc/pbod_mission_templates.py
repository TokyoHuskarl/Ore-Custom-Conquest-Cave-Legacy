...
init_player_global_variables = ( #in pbod_common_triggers and custom_camera_triggers
  ...
  #(ti_after_mission_start, 0, 0, [], [ 
	...
	#(agent_get_horse, ":horse", "$fplayer_agent_no"),  ## CC-D move
	#(agent_set_slot, "$fplayer_agent_no", slot_agent_horse, ":horse"),  ## CC-D move
  ])
...
## CC-D begin: rewrite weather penalty
init_weather_effects = ( #in pbod_battle_triggers
#  ti_before_mission_start, 0, 0, [(party_slot_ge, "p_main_party", slot_party_pref_wp_prof_decrease, 1)], [
...
#    (call_script, "script_weather_affect_proficiency", reg0, reg1), #from change_rain_or_snow
  ti_on_agent_spawn, 0, 0, [(party_slot_ge, "p_main_party", slot_party_pref_wp_prof_decrease, 15)], [
    (store_trigger_param_1, ":agent"),
    
    (try_begin),
      (agent_is_non_player, ":agent"),
      (party_get_slot, ":penalty", "p_main_party", slot_party_pref_wp_prof_decrease),
	  #occc perk and bonus begin
	  (agent_get_accuracy_modifier, ":def_acc", ":agent"),
	  (store_sub,":accuracy",":def_acc",100),
	  (val_add,":accuracy",":penalty"),
      (agent_set_accuracy_modifier, ":agent",  ":accuracy"),
	  #occc perk and bonus end
    (else_try),  #player
      (ge, "$g_ccd_rain_amount", 20),
      (assign, ":rain_strength", "$g_ccd_rain_amount"),
      (val_sub, ":rain_strength", 20),
      (val_div, ":rain_strength", 5),
      (store_sub, ":penalty", 100, ":rain_strength"),
	  #occc perk and bonus begin
	  (agent_get_accuracy_modifier, ":def_acc", ":agent"),
	  (store_sub,":accuracy",":def_acc",100),
	  (val_add,":accuracy",":penalty"),
...
common_pbod_triggers = [
  ...
  #horse_whistle,  ## CC-D move
  shield_bash, 
  init_weather_effects,  ## CC-D
 ] 
...
split_troop_division_triggers = [
 ...
 (1, 0, 0, [(party_slot_eq, "p_main_party", slot_party_prebattle_customized_divisions, 1),(eq, "$g_ccd_pbod_stop_all", 0),], [  ## CC-D add option
    (try_begin),
...
prebattle_deployment_triggers  = [
 (ti_before_mission_start, 0, 0, [(party_slot_eq, "p_main_party", slot_party_prebattle_customized_deployment, 1)], [
	...
	#(val_add, ":num_of_stacks", 1),  ## NMCml fix
...
caba_order_triggers = [
    ...
    (0, 0, 0, [
        (eq, "$g_ccd_pbod_stop_all", 0),  ## CC-D add option
		...
		(party_set_slot, "p_main_party", slot_party_gk_order, 0),
		(neq, "$g_ccd_pbod_stop_menu", 1),  ## CC-D add
	...
	(0, 0, 0, [(this_or_next|key_clicked, "$key_order_9"),(key_clicked, key_escape),(eq, "$g_ccd_pbod_stop_all", 0),], [  ## CC-D add option
	(close_order_menu),  ## CC-D add: 1.161-
	(party_set_slot, "p_main_party", slot_party_gk_order, 0),(is_presentation_active, "prsnt_caba_order_display"),(presentation_set_duration, 0)]),
		
	(0, 0, 0, [(game_key_clicked, gk_order_1),(neg|main_hero_fallen),(eq, "$g_ccd_pbod_stop_all", 0),], [  ## CC-D add option
		#(store_mission_timer_c_msec, "$when_f1_first_detected"), #CABA?  ## CC-D PBOD 0.96.3 del
		(try_begin),
	...
	(0, 0, 0, [(game_key_clicked, gk_order_2),(neg|main_hero_fallen),(eq, "$g_ccd_pbod_stop_all", 0),], [  ## CC-D add option
		(try_begin),
			(neg|party_slot_eq, "p_main_party", slot_party_gk_order, gk_order_1),
			(neg|party_slot_eq, "p_main_party", slot_party_gk_order, gk_order_2),
			(neg|party_slot_eq, "p_main_party", slot_party_gk_order, gk_order_3),
	...
	(0, 0, 0, [(game_key_clicked, gk_order_3),(neg|main_hero_fallen),(eq, "$g_ccd_pbod_stop_all", 0),], [  ## CC-D add option
		(try_begin),
			(neg|party_slot_eq, "p_main_party", slot_party_gk_order, gk_order_1),
			(neg|party_slot_eq, "p_main_party", slot_party_gk_order, gk_order_2),
			(neg|party_slot_eq, "p_main_party", slot_party_gk_order, gk_order_3),
	...
	(0, 0, 0, [(game_key_clicked, gk_order_4),(neg|main_hero_fallen),(eq, "$g_ccd_pbod_stop_all", 0),], [  ## CC-D add option
		(try_begin),
			(neq, "$g_ccd_pbod_stop_menu", 1),  ## CC-D add
	...
	(0, 0, 0, [(game_key_clicked, gk_order_5),(neg|main_hero_fallen),(eq, "$g_ccd_pbod_stop_all", 0),], [  ## CC-D add option
		(try_begin),
			(party_slot_eq, "p_main_party", slot_party_gk_order, 0),
	...
	(0, 0, 0, [(game_key_clicked, gk_order_6),(neg|main_hero_fallen),(eq, "$g_ccd_pbod_stop_all", 0),], [  ## CC-D add option
	    (try_begin),
			(party_slot_eq, "p_main_party", slot_party_gk_order, 0),
    ...
    (0, 0, 0, [(key_clicked, "$key_order_7"),(neg|main_hero_fallen),(eq, "$g_ccd_pbod_stop_all", 0),], [ #f7  ## CC-D add option
	    (try_begin),
		    (party_slot_eq, "p_main_party", slot_party_gk_order, 0), 
    ...
    (0, 0, 0, [(key_clicked, "$key_order_8"),(neg|main_hero_fallen),(eq, "$g_ccd_pbod_stop_all", 0),], [ #F8  ## CC-D add option
	    (try_begin),
	...
	#occc additional commands shortcut begin
    (0, 0, 0, [(key_is_down, key_left_control),(key_is_down, key_left_shift),(key_clicked, key_semicolon),(eq, "$g_ccd_pbod_stop_all", 0),], [ #none
			(call_script, "script_order_end_active_order", "$fplayer_team_no"),
	]),



    (0, 0, 0, [(key_is_down, key_left_control),(key_is_down, key_left_shift),(key_clicked, key_comma),(eq, "$g_ccd_pbod_stop_all", 0),], [ #Spear Bracing
			(call_script, "script_order_set_display_text", begin + 12),
			(call_script, "script_order_sp_brace_begin_end", begin, "$fplayer_team_no"),	]),
			
    (0, 0, 0, [(key_is_down, key_left_control),(key_is_down, key_left_shift),(key_clicked, key_period),(eq, "$g_ccd_pbod_stop_all", 0),], [ #Skirmish
			(call_script, "script_order_set_display_text", begin + 8),
			(call_script, "script_order_skirmish_begin_end", begin, "$fplayer_team_no"),
	]),

    (0, 0, 0, [(key_is_down, key_left_control),(key_is_down, key_left_shift),(key_clicked, key_slash),(eq, "$g_ccd_pbod_stop_all", 0),], [ #Volley Fire!
			(call_script, "script_order_set_display_text", begin + 10),
			(call_script, "script_order_volley_begin_end", begin, "$fplayer_team_no"),#			

	]),

    (0, 0, 0, [(key_is_down, key_left_control),(key_clicked, key_semicolon),(neg|main_hero_fallen),(eq, "$g_ccd_pbod_stop_all", 0),], [ #none
			(call_script, "script_player_order_formations", mordr_charge),
	]),

	
    (0, 0, 0, [(neg|key_is_down, key_left_shift),(key_is_down, key_left_control),(key_clicked, key_m),(neg|main_hero_fallen),(eq, "$g_ccd_pbod_stop_all", 0),], [ #ranks
			(call_script, "script_division_reset_places"),
			(agent_get_position, pos49, "$fplayer_agent_no"),
			(try_for_range, ":division", 0, 9),
			    (class_is_listening_order, "$fplayer_team_no", ":division"),
    ...
    (0, 0, 0, [(neg|key_is_down, key_left_shift),(key_is_down, key_left_control),(key_clicked, key_comma),(neg|main_hero_fallen),(eq, "$g_ccd_pbod_stop_all", 0),], [ #shield wall
			(call_script, "script_division_reset_places"),
			(agent_get_position, pos49, "$fplayer_agent_no"),
			(try_for_range, ":division", 0, 9),
			    (class_is_listening_order, "$fplayer_team_no", ":division"),
    ...
    (0, 0, 0, [(neg|key_is_down, key_left_shift),(key_is_down, key_left_control),(key_clicked, key_period),(neg|main_hero_fallen),(eq, "$g_ccd_pbod_stop_all", 0),], [  #wedge
			(call_script, "script_division_reset_places"),
			(agent_get_position, pos49, "$fplayer_agent_no"),
			(try_for_range, ":division", 0, 9),
			    (class_is_listening_order, "$fplayer_team_no", ":division"),
    ...
    (0, 0, 0, [(neg|key_is_down, key_left_shift),(key_is_down, key_left_control),(key_clicked, key_slash),(neg|main_hero_fallen),(eq, "$g_ccd_pbod_stop_all", 0),], [  #square
			(call_script, "script_division_reset_places"),
			(agent_get_position, pos49, "$fplayer_agent_no"),
			(try_for_range, ":division", 0, 9),
				(class_is_listening_order, "$fplayer_team_no", ":division"),
...
field_ai_triggers = [
  ...
  (2, 0, 0, [(eq, "$g_ccd_pbod_stop_all", 0),(this_or_next|party_slot_eq, "p_main_party", slot_party_pref_wu_lance, 1),(this_or_next|party_slot_eq, "p_main_party", slot_party_pref_wu_harcher, 1),(party_slot_eq, "p_main_party", slot_party_pref_wu_spear, 1),(store_mission_timer_a, reg0),(gt, reg0, 4)],  ## CC-D add option
  ...
  (ti_on_agent_hit, 0, 0, [(party_slot_eq, "p_main_party", slot_party_pref_dmg_tweaks, 1)],
   ...
   [
	...
	#occc bug resolve with decap mod
	...
	(try_begin),
	    ...
	    (try_begin), #Horse Trample Buff
            ...
            (try_begin),      
                (lt, ":horse_charge", 20),  ## CC-D adjust CtA Lv: 18->20
				(val_div, ":horse_charge", 3),
                (val_max, ":damage", ":horse_charge"),
            (else_try),
                (is_between, ":horse_charge", 20, 30),  ## CC-D adjust CtA Lv: 18->20 25->30
	...
	(else_try), #Pike Buff
		...
		(gt, ":weapon", 0),
		(item_get_weapon_length, ":cur_weapon_len", ":weapon"),  #1.161
		(ge, ":cur_weapon_len", 150),  #1.161
		...
		(item_get_body_armor, ":armor", ":horse"),  #1.161
  ...
  (1, 0, 0, [(eq, "$g_ccd_pbod_stop_all", 0),(store_mission_timer_a, reg0),(gt, reg0, 3), ###GENERAL AI TRIGGER for SPECIAL ORDERS   ## CC-D add option
        (party_slot_eq, "p_main_party", slot_party_pref_spear_brace, 1),
       ], [
		(set_fixed_point_multiplier, 100),
	    ...
	    (try_for_range, ":team", 0, 4), #For AI
			...
			(try_begin), #Spear Bracing Decision-making
				...
				(this_or_next|eq, ":faction", "fac_kingdom_5"), #Rhodoks
				(this_or_next|eq, ":faction", "fac_kingdom_4"), #Nords
				(eq, ":faction", "fac_kingdom_11"), #Old Rhodoks	
				#(eq, ":faction", "fac_kingdom_1"), #occc Swadian landsknechts	
				(try_begin),
			...
			(try_begin), #Volley/Skirmish Decision Making
				(this_or_next|eq,"fac_kingdom_7"),
				(this_or_next|eq,"fac_kingdom_3"),
				(this_or_next|eq,"fac_kingdom_8"),
				(this_or_next|eq,"fac_kingdom_10"),
				(eq,"fac_kingdom_12"),
				(try_begin),
			...
			(else_try),
				...
				(eq, ":faction", "fac_kingdom_2"), #occc vaegir line-infantry
				...
				(try_begin),
					...
					(is_between, reg0, 2500, 9500),#1000->2500 7000->9500
					(team_set_order_listener, ":team", grc_archers),
  ...
  (0.7, 0, 0, [(eq, "$g_ccd_pbod_stop_all", 0),(call_script, "script_cf_order_active_check", slot_team_d0_order_skirmish)], [(call_script, "script_order_skirmish_skirmish")]),   ## CC-D: 0.5->0.7 add option
 
  (1, 0, 0, [(eq, "$g_ccd_pbod_stop_all", 0),(call_script, "script_cf_order_active_check", slot_team_d0_order_volley)], [  ## CC-D add option
...
## NMCml begin: use horse_whistle other place
...
## NMCml end
  ...
  (0.1, 0, 0, [(eq, "$g_ccd_pbod_stop_all", 0),(call_script, "script_cf_order_active_check", slot_team_d0_order_sp_brace)], [ #spearwall_trigger_1  ## CC-D add option
		(try_for_agents,":agent"),
           (agent_is_alive,":agent"),
           (agent_is_human,":agent"),
           ...
           (ge,":speartimer",20),
		   (item_get_weapon_length, ":spear_dist", ":weapon"), #CABA  #1.161
		  ...
		  # (display_message, "@Brace should hit"),
           ...
           (agent_play_sound,":victim","snd_metal_hit_low_armor_low_damage"),
  ...
  (0, 0, 0, [(this_or_next|game_key_clicked, gk_attack),(this_or_next|game_key_clicked, gk_defend), #spearwall_trigger_3
       ...
       [
        ...
        (assign, ":valid_weapon", 0),
        (agent_get_wielded_item, ":weapon", "$fplayer_agent_no", 0),
        ...
        (try_begin),
            ...
            (this_or_next|eq, ":anim", "anim_spearwall_bracing"),
            (eq, ":anim", "anim_spearwall_bracing_low"),
            (assign, ":valid_anim", 1),
        (else_try),
            (agent_get_animation, ":anim", "$fplayer_agent_no", 1),
            (this_or_next|eq, ":anim", "anim_spearwall_bracing"),
            (eq, ":anim", "anim_spearwall_bracing_low"),
            (assign, ":valid_anim", 1),
        (try_end),
...
bodyguard_triggers = [
 ...
 (ti_on_agent_spawn, 0, 0, [(party_slot_eq, "p_main_party", slot_party_pref_bodyguard, 1)], 
   [
	...
	## CC-D begin: for dialog
	(agent_set_slot, ":agent", slot_agent_bodyguard, 1),
	## CC-D end
   ]),
...
def modmerge_mission_templates(orig_mission_templates):
	from module_mission_templates import common_battle_tab_press, common_battle_check_victory_condition, common_battle_order_panel, common_battle_order_panel_tick, common_siege_check_defeat_condition, custom_commander_change_weather  ## NMCml add common_battle_order_panel_tick  ## CC-D: add ccd_init_variables->custom_commander_change_weather: rewrite weather penalty
	...
	for i in range(len(orig_mission_templates)):
		...
		## CC-D begin: reuse slot_party_pref_bc_charge_ko and add camera
		trigger_i = MissionTemplateWrapper(orig_mission_templates[i]).FindTrigger_i(1,4,0,[(main_hero_fallen)])
		if ( trigger_i != None ):
			trigger = orig_mission_templates[i][5][trigger_i]
			codeblock = TriggerWrapper(trigger).GetConsequenceBlock()
			pos = codeblock.FindLineMatching((set_show_messages, 0))
			if (trigger != common_siege_check_defeat_condition):
				codeblock.InsertBefore(pos, [(party_slot_eq, "p_main_party", slot_party_pref_bc_charge_ko, 1)])
			pos = codeblock.FindLineMatching((neq, ":cur_order", mordr_charge))
			if (trigger != common_siege_check_defeat_condition):
				codeblock.InsertAfter(pos+4, [(team_set_order_listener, ":player_team", grc_everyone),(call_script, "script_player_order_formations", mordr_charge)])
			pos = codeblock.FindLineMatching((assign, "$ccc_help_msg_str", "str_ccd_help_message_dc_smooth"))
			if (trigger != common_siege_check_defeat_condition):
				codeblock.InsertAfter(pos, ccd_pbod_death_cam_addon)
		## CC-D end
		## CC-D begin
		trigger_i = MissionTemplateWrapper(orig_mission_templates[i]).FindTrigger_i(3,0,0)
		if ( trigger_i != None ):
			trigger = orig_mission_templates[i][5][trigger_i]
			codeblock = TriggerWrapper(trigger).GetConditionBlock()
			pos = codeblock.FindLineMatching((eq, "$g_ccd_no_run_away", 0))
			codeblock.InsertBefore(pos, [(this_or_next|party_slot_eq, "p_main_party", slot_party_pref_formations, 1),])
			trigger_i += 1  #get next 3,0,0 trigger by brute force
			trigger = orig_mission_templates[i][5][trigger_i]
			codeblock = TriggerWrapper(trigger).GetConditionBlock()
			pos = codeblock.FindLineMatching((eq, "$g_ccd_no_run_away", 0))
			codeblock.InsertBefore(pos, [(this_or_next|party_slot_eq, "p_main_party", slot_party_pref_formations, 1),])
	...
	try:
		...
		## NMCml begin: toggle for CC order panel
		codeblock = TriggerWrapper(common_battle_order_panel_tick).GetConsequenceBlock()
		pos = codeblock.FindLineMatching((neg|is_presentation_active, "prsnt_mini_map"))
		codeblock.InsertAfter(pos,[(neg|is_presentation_active, "prsnt_caba_order_display"),(neg|is_presentation_active, "prsnt_caba_camera_mode_display")])
		## NMCml end
		
		## CC-D begin: reuse slot_party_pref_bc_charge_ko and add camera
		codeblock = TriggerWrapper(common_siege_check_defeat_condition).GetConsequenceBlock()
		pos = codeblock.FindLineMatching((neq, ":cur_order", mordr_charge))
		codeblock.InsertAfter(pos+4, [(team_set_order_listener, ":player_team", grc_everyone),(call_script, "script_player_order_formations", mordr_charge)])
		pos = codeblock.FindLineMatching((assign, "$ccc_help_msg_str", "str_ccd_help_message_dc_smooth"))
		codeblock.InsertAfter(pos, ccd_pbod_death_cam_addon)
		## CC-D end
		## CC-D begin: rewrite weather penalty
		#codeblock = TriggerWrapper(ccd_init_variables).GetConsequenceBlock()
		#pos = codeblock.FindLineMatching((eq, "$g_rand_rain_limit", -1))
		#codeblock.InsertAfter(pos,[(party_slot_eq, "p_main_party", slot_party_pref_wp_prof_decrease, 0)])
		codeblock = TriggerWrapper(custom_commander_change_weather).GetConditionBlock()
		pos = codeblock.FindLineMatching((neq, "$g_rand_rain_limit", -1))
		codeblock.InsertBefore(pos,[(this_or_next|party_slot_eq, "p_main_party", slot_party_pref_wp_prof_decrease, 1)])
		## CC-D end
