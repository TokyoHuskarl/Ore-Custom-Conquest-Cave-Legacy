...
scripts = [
 ...
 ("prebattle_set_default_prefs", [ 
	(party_set_slot, "p_main_party", slot_party_pref_wu_lance, 0),  ## CC-D 1->0
	(party_set_slot, "p_main_party", slot_party_pref_wu_harcher, 0),  ## CC-D 1->0
	(party_set_slot, "p_main_party", slot_party_pref_wu_spear, 0),  ## CC-D 1->0
	(party_set_slot, "p_main_party", slot_party_pref_dmg_tweaks, 0),  ## CC-D 1->0
	
	(party_set_slot, "p_main_party", slot_party_pref_bodyguard, 1),
	(party_set_slot, "p_main_party", slot_party_pref_bc_continue, 1),
	(party_set_slot, "p_main_party", slot_party_pref_bc_charge_ko, 1),  ## CC-D 0->1
	...
	(party_set_slot, "p_main_party", slot_party_pref_wp_prof_decrease, 0),  ## CC-D 1->0
    ]),
 ...
 ("weapon_use_classify_agent", [ 
    ...
    (try_begin),
		...
		(try_begin),
			...
			(neg|troop_is_mounted, ":troop"), #Not Mounted  ## CC-D fix
		...
		(else_try),	
			...
			(try_begin),
				...
				(try_begin),
					(gt, ":wielded", 0),
					(item_has_property, ":wielded", itp_couchable),  #1.161
	   ...
	   # actually have a lance equipped.  Otherwise this does nothing.
					...
					(try_for_range, ":i", ek_item_0, ":end"),
						(agent_get_item_slot, ":item", ":agent", ":i"),
						(gt, ":item", 0),
						(item_has_property, ":item", itp_couchable),  #1.161
	...
	(try_for_agents, ":agent"),
		...
		(try_for_range, ":i", ek_item_0, ":end"),
			...
			(try_begin),
				...
				## CC-D begin
				(this_or_next|eq, ":weapontype", itp_type_pistol),
				(this_or_next|eq, ":weapontype", itp_type_musket),
				## CC-D end
...
#Shield Bash Script - Xenoargh (Blood & Steel); edited/reworked per comments in the thread, etc, by Caba
 ("cf_shield_bash",
   [  
	...
	(item_get_weapon_length, ":shield_width", ":shield_item"),  #1.161
	...
	(try_begin),
		...
		#occc power bash perk start
		(try_begin),
			(agent_slot_eq, "$fplayer_agent_no", slot_agent_can_powerbash, 1),
			(agent_set_animation, ":victim", "anim_strike_knockback_rolling", 0),
			(agent_deliver_damage_to_agent, "$fplayer_agent_no", ":victim", 10),
		(else_try),
 ...
 ("prebattle_agent_fix_division",
   [
	...
	(try_begin),
		...
		(agent_get_troop_id, ":troop", ":agent"),
		(try_begin),
			(troop_is_mounted, ":troop"),  ## CC-D fix
...
#      ## CC-D begin
#      (assign, "$g_ccd_rain_amount", ":rand_strength"),
#    (else_try),
...
#          (display_message, "@Weather penalty {reg2}%"),
#        (try_end),
#        ## CC-D end
#    (try_end),	
#   ]),
...
#		(else_try),
#			(eq, ":n", 3),
#			(assign, ":proficiency", wpt_firearm),
#			#(assign, ":prof_item_type", itp_type_pistol),
#			(assign, ":prof_slot", slot_troop_orig_wpt_firearm),
#		## CC-D End
#		(try_end),
#		
...
#			(val_add, ":prof_slot", 4), #bump to pnty_wpt slot  ## CC-D 3->4
...
#				(try_begin),
#					(troop_is_hero, ":troop_id"),  
#					(store_add, ":pnty_slot", ":prof_slot", 4), #trackign penalty level allows heroes WP gains during battle   ##BUG in Floris 2.5b2, missing argument  ## CC-D 3->4
...
#   ]), 
  ("set_weather_effects",
   [
    (try_begin),
      (party_slot_ge, "p_main_party", slot_party_pref_wp_prof_decrease, 1),
      
      (assign, ":penalty", 0),
      
      (try_begin),  #night or fog
        (is_currently_night),
        (val_add, ":penalty", 15),
      (else_try),
        (get_global_haze_amount, ":fog"),
        (gt, ":fog", 90),  #fog effect occur
        (val_add, ":penalty", 15),
      (try_end),
      ...
      (try_begin),
        ...
        (val_add, ":penalty", ":rain_strength"),  #max:16
        (neq, ":terrain_type", rt_snow),
        (neq, ":terrain_type", rt_snow_forest),  #again when rain
        (val_add, ":penalty", ":rain_strength"),  #max:16
      (else_try),
        (assign, ":rain_strength", 0),
      (try_end),
      
      (val_min, ":penalty", 60),  #51->60: current max:47=[15or15]+16+16
      (try_begin),
        (gt, ":penalty", 0),
        (assign, reg2, ":penalty"),
        (assign, reg3, ":rain_strength"),  #player only rain
        (display_message, "@Weather penalty {reg2}% - affect player {reg3}%"),
        (val_sub, ":penalty", 100),
        (val_abs, ":penalty"),
        (party_set_slot, "p_main_party", slot_party_pref_wp_prof_decrease, ":penalty"),  #50-99
      (else_try),
        (party_set_slot, "p_main_party", slot_party_pref_wp_prof_decrease, 10),  #10:no effect
      (try_end),
    (try_end),
   ]),
## CC-D end
...
scripts_directives = [
	...
	## CC-D begin: rewrite weather penalty
	[SD_OP_BLOCK_INSERT, "change_rain_or_snow", D_SEARCH_FROM_BOTTOM | D_SEARCH_LINENUMBER | D_INSERT_AFTER, 0, 0,
	[
		(call_script, "script_set_weather_effects"),
	]], #ADD TO END
	## CC-D end
] 
