...
#menu_world_map_soldier
    ("world_map_soldier",mnf_enable_hot_keys,  ## NMCml FL: enable hot key: 0->mnf_enable_hot_keys
    "What do you need to do soldier?",
    "none",
    [
	 (set_background_mesh, "mesh_pic_soldier_world_map"),  ## NMCml FL: native picture: mesh_pic_soldier_world_map->mesh_pic_camp
	 #(troop_get_slot, "$enlisted_party", "$enlisted_lord", slot_troop_leaded_party), #CABA - to refresh it? maybe not necessessary  ## CC-D for caravan

	 #occc contacting enemy alert sound
	...
	[
		("join_commander_battle",[
			(party_get_battle_opponent, ":commander_opponent", "$enlisted_party"),
			(gt, ":commander_opponent", 0),
			(str_store_party_name, 1,":commander_opponent"),
			(neg|troop_is_wounded, "trp_player"),  ## NMCml FL: stop when wounded and surrender
		],"Follow the commander into battle. Against {s1}",[
			...
			(try_begin),
				(eq, "$g_fl_no_auto_equip", 0),  ## CC-D add: stop auto equip
				(neg|party_slot_eq, "$enlisted_party", slot_party_type, spt_kingdom_caravan),  ## CC-D for caravan
		...
		## NMCml FL begin: stop when wounded and surrender
		("cant_join_commander_battle",[
			(party_get_battle_opponent, ":commander_opponent", "$enlisted_party"),
			(gt, ":commander_opponent", 0),
			(troop_is_wounded, "trp_player"),
			(disable_menu_option),
		],"Cant join this battle.",[]),
		#slack off: refer from TMP(Freelancer: Slack off (PBOD))
		("join_slack_off", [
			(eq, "$freelancer_state", 1),
			(party_get_battle_opponent, ":commander_opponent", "$enlisted_party"),
			(gt, ":commander_opponent", 0),
			(neg|troop_is_wounded, "trp_player"),
			(call_script, "script_party_count_fit_for_battle", "$enlisted_party"),
			(gt, reg0, 0),
		...
		("surrender", [
			...
			(assign, "$flag_slack_off", 0),
			(call_script, "script_encounter_init_variables"),
			(party_get_battle_opponent, ":commander_opponent", "$enlisted_party"),
			(assign, "$g_enemy_party", ":commander_opponent"),
			(assign, "$g_ally_party", "$enlisted_party"),
			(party_set_slot, "p_freelancer_party_backup", slot_party_last_in_combat, 1), #needed to catch post-battle and detach any attached parties
		...
		("commander",[(party_get_battle_opponent, ":commander_opponent", "$enlisted_party"),(lt, ":commander_opponent", 0),(neg|party_slot_eq, "$enlisted_party", slot_party_type, spt_kingdom_caravan),],  ## CC-D
		   "Request audience with your commander.",
        [(jump_to_menu, "mnu_commander_aud"),]),
		
		("revolt",[(neg|party_slot_eq, "$enlisted_party", slot_party_type, spt_kingdom_caravan),],"Revolt against the commander!",  ## CC-D
        [(jump_to_menu, "mnu_ask_revolt"),]),
		
		("desert",[(neg|party_slot_eq, "$enlisted_party", slot_party_type, spt_kingdom_caravan),],"Desert the army.(keep equipment but lose relations)",  ## CC-D
        ...
        [
          (call_script, "script_event_player_discharge_caravan"),
		...
		("return_to_duty",[
			...
			(troop_is_wounded, "trp_player"),
		## NMCml FL begin: stop when wounded and surrender
			(str_store_string, s5, "@Return to duty."),
			(try_begin),
			  (gt, ":commander_opponent", 0),
			  (troop_is_wounded, "trp_player"),
			  (str_store_string, s5, "@Wait for your recovering..."),
			(try_end),
		],"{s5}",
		## NMCml FL end
        [(change_screen_map),
...
#menu_aud_with_commander 
  (
    ...
    "none",
    [(set_background_mesh, "mesh_pic_soldier_world_map"),(str_store_troop_name, s6, "$enlisted_lord")],  ## NMCml FL: native picture: mesh_pic_soldier_world_map->mesh_pic_camp
    [
      ("continue",[],
       "Continue...",
       [
    ...
    "none",
    [(set_background_mesh, "mesh_pic_soldier_rebel"),(str_store_troop_name, s6, "$enlisted_lord")],[  ## NMCml FL: native picture: mesh_pic_soldier_rebel->mesh_pic_deserters
    ...
    "none",
    [
        (set_background_mesh, "mesh_pic_soldier_rebel"),  ## NMCml FL: native picture: mesh_pic_soldier_rebel->mesh_pic_deserters
    ...
    "none",
    [(set_background_mesh, "mesh_pic_soldier_desert"),],[  ## NMCml FL: native picture: mesh_pic_soldier_desert->mesh_pic_deserters
   ...
   ("upgrade_path",0,
    ...
    [
        ...
        "{s67}",[
            ...
            (add_quest_note_from_sreg, "qst_freelancer_enlisted", 3, s5, 1),
            (change_screen_map),]),
## CC-D begin: add view upgrade tree
        ("action_view_upgrade_trees", [
            (store_troop_faction, ":commander_faction", "$enlisted_lord"),
            (is_between, ":commander_faction", npc_kingdoms_begin, npc_kingdoms_end),
        ],"View upgrade trees.", [
            (call_script, "script_game_troop_upgrades_button_clicked", "$player_cur_troop"),
        ]),
## CC-D end
    ]),
#+freelancer end
...
def modmerge_game_menus(orig_game_menus, check_duplicates = False):
	...
	##FLORIS ONLY END
		...
		codeblock = GameMenuWrapper(orig_game_menus[find_i]).GetMenuOptions()
		codeblock.extend(join_wounded_freelancer)	
		## NMCml FL begin: fix siege defence from TMP
		find_i = list_find_first_match_i( orig_game_menus, "siege_started_defender" )
		codeblock = GameMenuWrapper(orig_game_menus[find_i]).GetOpBlock()
		pos = codeblock.FindLineMatching((eq,":attackers_retreat",1))
		codeblock.InsertBefore(pos, [(neq,"$freelancer_state",1)])
		## NMCml FL end
		find_i = list_find_first_match_i( orig_game_menus, "besiegers_camp_with_allies" )
		codeblock = GameMenuWrapper(orig_game_menus[find_i]).GetMenuOptions()
		codeblock.extend(join_wounded_freelancer)
