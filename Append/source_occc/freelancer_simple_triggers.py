...
#+freelancer start
  #  WEEKLY PAY AND CHECKS FOR UPGRADE
    ...
    (24 * 7, [
		...
		### NMCml FL begin: non Diplo pay time
		##Following two lines only necessary if Diplomacy not included
		#(store_current_hours, "$g_next_pay_time"),
		#(val_add, "$g_next_pay_time", 24 * 7),		
		##End non-Diplomacy lines
		### NMCml FL end
...
#  HOURLY CHECKS
    ...
    (1,[
        ...
        (try_begin),
           (lt, ":num_food", 2),
           (troop_add_item, "trp_player", "itm_bread"),   ## NMCml FL: bread fix: itm_trade_bread->itm_bread
        (try_end),
    ]),
	
## NMCml FL begin: stop when wounded and surrender: refer from TMP(Freelancer: Slack off (PBOD))
#To check flag_slack_off
  (16,
    [
      (neq, "$flag_slack_off", 0),
...
## CC-D begin :Freelancer pause in town from Tiny Mod Patcher
  (24,
    [
      (gt, "$g_fl_pause_in_town", 1),
      (val_sub, "$g_fl_pause_in_town", 1),
    ]),
## CC-D end
  #+freelancer end  
]
