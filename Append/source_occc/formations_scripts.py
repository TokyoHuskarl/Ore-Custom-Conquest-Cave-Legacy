...
# #Formations Scripts	  
  ...
  ("form_archers", [
	...
	(assign, ":shift_y", 75), #caba - begin
	(try_begin),
		(store_add, ":slot", slot_team_d0_formation_num_ranks, ":fdivision"),
		(team_get_slot, ":num_ranks", ":fteam", ":slot"),
		(is_between, ":num_ranks", 1, 6),
	(else_try),
		(assign, ":num_ranks", 3),
		(team_set_slot, ":fteam", ":slot", ":num_ranks"),
	(try_end),
 	(val_sub, ":num_ranks", 1),
	(val_mul, ":num_ranks", ":shift_y"),
	(store_mul, ":neg_shift_y", ":num_ranks", -1),
	...
	(try_for_agents, ":agent"),
		...
		(try_begin),
			...
			(val_add, ":total_move_y", ":shift_y"),
			(val_add, ":rank", 1),
			(try_begin),
				(le, ":total_move_y", ":num_ranks"),
				(position_move_y, pos1, ":shift_y", 0),
			(else_try),			    
				(position_move_y, pos1, ":neg_shift_y", 0),
  ...
  ("form_infantry", [
	...
	(else_try),
		...
		(else_try),
			...
			(team_set_slot, ":fteam", ":slot", ":num_ranks"),
		(try_end), #caba
		(store_div, ":rank_dimension", ":num_troops", ":num_ranks"),		#basic three ranks
		(val_add, ":rank_dimension", 1),		
		(assign, ":max_level", 0),
		(try_for_agents, ":agent"),
			(call_script, "script_cf_valid_formation_member", ":fteam", ":fdivision", ":fleader", ":agent"),
		...
		(try_for_range, ":weap_group", 0, 4),
			...
			(try_for_agents, ":agent"),
				...
				(try_begin),
					...
					(item_get_weapon_length, ":weapon_length", ":agent_weapon"),  #1.161
				(else_try),
  ...
  ("get_default_formation", [
	...
	(else_try),
	    (this_or_next|eq, ":ffaction", fac_kingdom_12),#occc old vaegirs
		(eq, ":ffaction", fac_kingdom_2),	#Vaegirs
		(assign, reg0, formation_ranks),
	(else_try),
	    (this_or_next|eq, ":ffaction", fac_kingdom_10),#occc sunset
		(eq, ":ffaction", fac_kingdom_3),	#Khergit
		(assign, reg0, formation_none),	#Khergit have underdeveloped infantry
	(else_try),
	    (this_or_next|eq, ":ffaction", fac_kingdom_14),#occc albion
	    (this_or_next|eq, ":ffaction", fac_kingdom_11),#occc old rhodok
		(eq, ":ffaction", fac_kingdom_4),	#Nords
		(assign, reg0, formation_shield),
	(else_try),
	    (this_or_next|eq, ":ffaction", fac_kingdom_8),#occc Hellas
		(eq, ":ffaction", fac_kingdom_5),	#occc  Rhodok
		(assign, reg0, formation_square),
		#(call_script, "script_occc_team_get_greece_infantry_percentages", ":fteam", 1),
		#(try_begin),
		#	(ge, reg0, 40),
		#	(assign, reg0, formation_square),#pseudo-phalanx
		#(else_try),
		#	(assign, reg0, formation_ranks),
		#(try_end),
	(else_try),
	    (this_or_next|eq, ":ffaction", fac_kingdom_9),#occc taikou
		(eq, ":ffaction", fac_kingdom_6),	#occc Sarranid 
		#(assign, reg0, formation_ranks),
	...
	(else_try),
		...
		#occc start
	(else_try),
	(eq, ":ffaction", fac_kingdom_7),	#Dark knights 
	(assign, reg0, formation_ranks),
		#occc end
	(else_try),
		(assign, reg0, formation_none),	#riffraff don't use formations
	(try_end),
	## CC-D begin: add unique
	(try_begin),
		(eq, ":ffaction", fac_kingdom_4),#nord
		(call_script, "script_ccd_team_get_barbarian_percentages", ":fteam", 1),
		(try_begin),
			(ge, reg0, 30),
		...
		(else_try),
			(assign, reg0, formation_shield),
		(try_end),
		#occc begin ...Nords try desperate charge against line infantries
		(try_begin),
			(eq, "$ai_team_1", ":fteam"),
			(ge, "$ai_team_2",0),
			(team_get_slot, ":against_faction", "$ai_team_2", slot_team_faction),
			(try_begin),
				(this_or_next|eq,":against_faction",fac_kingdom_7),
				(eq,":against_faction",fac_kingdom_2),
				(assign, reg0, formation_none),
			(try_end),
		(else_try),
			(eq, "$ai_team_2", ":fteam"),
			(ge, "$ai_team_1",0),
			(team_get_slot, ":against_faction", "$ai_team_1", slot_team_faction),
			(try_begin),
				(this_or_next|eq,":against_faction",fac_kingdom_7),
				(eq,":against_faction",fac_kingdom_2),
				(assign, reg0, formation_none),
	...
	(else_try),
		(team_get_leader, ":leader_agent_no", ":fteam",),
		(ge, ":leader_agent_no", 0),
		(agent_get_troop_id, ":leader_troop_no", ":leader_agent_no",),
		(this_or_next|eq, ":leader_troop_no", "trp_ccc_shougun_1"),
		(this_or_next|eq, ":leader_troop_no", "trp_ccc_bandit_leader"),
		(eq, ":leader_troop_no", "trp_ccc_bandit_king"),
	...
	(try_begin), #CABA additions
		...
		(try_begin),
			...
			(try_for_range, ":item_slot", ek_item_0, ek_head),
				...
				(this_or_next|neg|item_has_property, ":item", itp_two_handed),  #1.161
				(this_or_next|eq, ":force_shield", 0),
				(eq, ":shield", itm_no_item),
				
				(try_begin),
					(neq, ":force_length", 0),
					(item_get_weapon_length, ":item_length", ":item"),  #1.161
					(try_begin),
  ...
  # script_formation_change_num_ranks by Caba'drin
  # Input: team, division, 1 to increase or -1 to decrease (typically)
  # Output: none #change will take effect on next 'reform'
  ("formation_change_num_ranks", [
  	(store_script_param, ":fteam", 1),
	(store_script_param, ":fdivision", 2),
	(store_script_param, ":change", 3),
	(try_begin),
		(store_add, ":slot", slot_team_d0_formation, ":fdivision"),
		(team_get_slot, ":formation", ":fteam", ":slot"),
		(this_or_next|eq, ":formation", formation_ranks),
		(eq, ":formation", formation_shield),
		(store_add, ":slot", slot_team_d0_formation_num_ranks, ":fdivision"),
		(team_get_slot, ":num_ranks", ":fteam", ":slot"),
		(val_add, ":num_ranks", ":change"),
		(val_clamp, ":num_ranks", 1, 6),
	    (team_set_slot, ":fteam", ":slot", ":num_ranks"),
	(try_end),
  ]),
  ...
  ("store_battlegroup_data", [
	...
	(try_for_agents, ":cur_agent"),
		...
		(try_begin),
			...
			(item_get_weapon_length, ":cur_weapon_length", ":cur_weapon"),  #1.161

			(item_get_thrust_damage, reg0, ":cur_weapon"),  #1.161
			(item_get_swing_damage, ":cur_swing", ":cur_weapon"),  #1.161
			(ge, ":cur_swing", reg0),  #1.161
		...
		#this section is "the agent-squared loop" that causes the whole mod to develop jitters as more agents are added to the battle
		#the engine must also track this info, so it's a huge shame there's no op to retrieve it
		#however, I've added a player option to disable (making AI not so smart)
