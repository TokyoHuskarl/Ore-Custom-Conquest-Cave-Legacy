...
AI_triggers = [  
	...
	(0, AI_Delay_For_Spawn, ti_once, [(party_slot_eq, "p_main_party", slot_party_pref_formations, 1)], [
		...
		(call_script, "script_field_ai_start_positions"), #CABA
	]),

	(0.3, 0, 0, [(party_slot_eq, "p_main_party", slot_party_pref_formations, 1),(eq, "$g_ccd_pbod_stop_all", 0),], [	  ## CC-D: 0.1->0.3 add option
		...
		(try_begin),
			(main_hero_fallen),
			(call_script, "script_store_battlegroup_data"),	#else, done in formations trigger; performance
		(try_end),
		(try_begin),	
...
def modmerge_formAI_mission_templates(orig_mission_templates):
	...
	trigger = MissionTemplateWrapper(orig_mission_templates[find_i]).FindTrigger(0, 0, ti_once, [(store_mission_timer_a,":mission_time"),(ge,":mission_time",2)],
        [(call_script, "script_ccc_select_battle_tactic"),(call_script, "script_ccc_battle_tactic_init")]) ## CC-D
	...
	#
	trigger = MissionTemplateWrapper(orig_mission_templates[find_i]).FindTrigger(1.1, 0, 0, [(store_mission_timer_a,":mission_time"),(ge,":mission_time",20),(call_script, "script_ccc_battle_tactic_apply"),], [])  ## CC-D (5, 0, 0, [(store_mission_timer_a,":mission_time"),(ge,":mission_time",3),(call_script, "script_battle_tactic_apply")], []) 
	codeblock = trigger.GetConditionBlock()
	codeblock.InsertBefore(0, [(neg|party_slot_eq, "p_main_party", slot_party_pref_formations, 1)]) ## PBOD - Formations AI NOT active
